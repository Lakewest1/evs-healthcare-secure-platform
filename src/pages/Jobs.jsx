...
//  2. Domestic Worker: pay "£13–£16/hr", payValue 14.5
//  3. Verified lucide import removed → CheckCircle used throughout
//  4. isMobile initialised to false (SSR-safe), set via matchMedia in useEffect
//  5. All resize listeners use AbortController for clean teardown
//  6. Carousel auto-play timer cleaned up properly on unmount
//  7. Carousel nav buttons have aria-label
//  8. Progress bar keyed to progressKey to prevent mid-slide glitch
//  9. Partners IS_MOBILE constant replaced with useIsMobile() hook (SSR-safe)
// ─────────────────────────────────────────────────────────────────────────────
...
function useIsMobile(breakpoint = 768) {
  const [isMobile, setIsMobile] = useState(false); // false on SSR
  useEffect(() => {
...
// FIX 1 + 2: Dollar SVG removed. All pay displayed as plain £ text.
//            Domestic Worker corrected: £13–£16/hr, payValue 14.5
const JOBS_DATA = [
  {
    id: 1,
  ...
  },
  {
    ...
    type: "Flexible Hours",
    location: "Across London",
    // FIX 2: was "£14.50–£12/hr" (inverted range) → corrected
    pay: "£13–£16/hr",
    payValue: 14.5,
    urgent: false,
    icon: Star,
  ...
  return (
    ...
    >
      ...
      <div className="job-card-details">
        ...
        {/*
          FIX 1: Pay row — removed the dollar-sign SVG entirely.
          The pound sign is part of the pay string (e.g. "£26–£38/hr")
          so no icon is needed. A simple £ prefix label replaces it.
        */}
...
function JobsSection() {
  ...
  return (
    <section
      className="jobs-page"
      aria-labelledby="jobs-page-heading"
      id="jobs"
    >
    ...
    >
      ...
      <div style={{ marginBottom: 12 }}>
        <Quote
          size={24}
          strokeWidth={1.5}
          style={{ color: "rgba(196,151,42,0.3)" }}
          aria-hidden="true"
        />
      </div>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            ...
            {testimonial.verified && (
              // FIX 3: Verified doesn't exist in lucide-react — CheckCircle used
              <span
              ...
              >
                <CheckCircle size={8} aria-hidden="true" /> Verified
              </span>
            )}
          </div>
          <div
          ...
          >
            ...
            <span aria-hidden="true" style={{ color: "#cbd5e1", fontSize: 8 }}>•</span>
            <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
              <Clock size={9} style={{ color: T.mutedSoft }} aria-hidden="true" />
              <span
                style={{
                  fontFamily: "'Inter', sans-serif",
                  color:      T.mutedSoft,
...
function TestimonialCarousel() {
  ...
  const isMobile     = useIsMobile();        // FIX 4: SSR-safe
  const timerRef     = useRef(null);
  const shouldReduce = useReducedMotion();
  const total        = TESTIMONIALS.length;

  // FIX 6: auto-play cleaned up correctly
  useEffect(() => {
    if (!autoActive || shouldReduce) return;
    timerRef.current = setInterval(() => {
      setIndex((p) => (p + (isMobile ? 1 : VISIBLE_COUNT)) % total);
      setProgressKey((k) => k + 1);
    }, AUTO_MS);
    return () => clearInterval(timerRef.current);
  }, [autoActive, isMobile, total, shouldReduce]);

  const pause = useCallback(() => {
    setAutoActive(false);
    clearInterval(timerRef.current);
    const t = setTimeout(() => setAutoActive(true), RESUME_MS);
    return () => clearTimeout(t);
  }, []);

  const prev = useCallback(() => {
    pause();
    setIndex((p) => (p - (isMobile ? 1 : VISIBLE_COUNT) + total) % total);
    setProgressKey((k) => k + 1);
  }, [pause, isMobile, total]);
  ...
  const dotCount = isMobile
    ? total
    : Math.ceil(total / VISIBLE_COUNT);

  const activeDot = isMobile
    ? index % total
    : Math.floor(index / VISIBLE_COUNT);

  return (
    <div style={{ position: "relative" }}>
      {/* FIX 7: aria-label on carousel nav buttons */}
      <button
        onClick={prev}
      ...
      >
        <AnimatePresence mode="wait">
          {visible.map((t, i) => (
            <motion.div
              key={`${index}-${i}`}
              initial={shouldReduce ? false : { opacity: 0, y: isMobile ? 0 : 20, x: isMobile ? 50 : 0 }}
              animate={{ opacity: 1, y: 0, x: 0 }}
              exit={shouldReduce ? {} : { opacity: 0, y: isMobile ? 0 : -20, x: isMobile ? -50 : 0 }}
              transition={{ duration: isMobile ? 0.5 : 0.4, delay: i * 0.08, ease: EASE }}
            >
              <TestimonialCard testimonial={t} isActive />
            </motion.div>
          ))}
        </AnimatePresence>
      </div>

      {/* Progress bar — FIX 8: keyed to progressKey so it re-mounts on every slide */}
      {!shouldReduce && (
      ...
      >
        {Array.from({ length: dotCount }).map((_, i) => (
          <button
            ...
            onClick={() =>
              goTo(isMobile ? i : i * VISIBLE_COUNT)
            }
    ...
    >
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        >
          ...
          {/* Rating badge — integrated, not standalone card */}
          <div
            style={{
              display:      "inline-flex",
              alignItems:   "center",
            ...
            aria-label={`5 out of 5 stars — ${TESTIMONIALS.length} reviews`}
          >
        ...
        {/* Trust indicators — desktop only via CSS */}
        <motion.div
          initial={shouldReduce ? false : { opacity: 0, y: 20 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Users size={15} color={T.gold} aria-hidden="true" />
            <span
              style={{
                fontFamily: "'Inter', sans-serif",
                fontSize:   13,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <ThumbsUp size={14} color={T.gold} aria-hidden="true" />
            <span
              style={{
                fontFamily: "'Inter', sans-serif",
                fontSize:   13,
        ...
        >
          ...
          />
          <motion.div
            animate={shouldReduce ? {} : { scale: [1, 1.3, 1], opacity: [0.4, 1, 0.4] }}
...
// Framer Motion variants for Partners header
...
function PartnersSection() {
  ...
  // FIX 9: IS_MOBILE constant replaced with SSR-safe hook
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 2 }}>
        ...
        >
          <motion.div
            ...
            whileHover={isMobile || shouldReduce ? {} : { scale: 1.03, y: -2 }}
            style={{
              display:      "flex",
              alignItems:   "center",
              gap:          8,
...
// ─────────────────────────────────────────────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
//  ROOT EXPORT — Jobs → Testimonials → Partners
// ══════════════════════════════════════════════════════════════════════════════
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /*
          FIX 1: Pay row — .job-card-pay-symbol replaces the dollar SVG.
          Renders a small gold "£" label at the same size and weight as
          the detail icons so the row stays visually aligned.
        */
        ...
        .carousel-nav:hover { background: #C4972A; }
        .carousel-nav:hover svg { stroke: #fff !important; }
        ...
        .carousel-dot.active { width: 32px; background: #C4972A; }
        .trust-indicators {
          margin-top: 48px;
          display: flex;
          justify-content: center;
